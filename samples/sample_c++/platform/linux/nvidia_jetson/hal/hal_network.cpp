...
chcnav_return_code_t hal_network_init(const char *ip_addr, const char *net_mask,
  ...
  snprintf(cmd_str, sizeof(cmd_str), "ifconfig %s %s net_mask %s",
           LINUX_NETWORK_DEV, ip_addr, net_mask);
  ret = system(cmd_str);
  if (ret != CHCNAV_RETURN_OK) {
    std::cout << "Hal_network_init can't config the ip address of network."
              << std::endl;
    return CHCNAV_RETURN_ERR_SYSTEM;
  }
...
hal_network_deinit(chcnav_network_handle_t network_handle) {
  return CHCNAV_RETURN_OK;
}

chcnav_return_code_t hal_network_get_device_info(
