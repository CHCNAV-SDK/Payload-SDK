...
chcnav_return_code_t osal_task_create(const char *name,
  ...
  *task = malloc(sizeof(pthread_t));
  ...
  result = pthread_create(reinterpret_cast<pthread_t *>(task), nullptr,
                          task_func, arg);
  if (result != 0) {
  ...
  result =
      pthread_setname_np(*(reinterpret_cast<pthread_t *>(task)), thread_name);
  if (result != 0) {
    return CHCNAV_RETURN_ERR_SYSTEM;
  }
...
chcnav_return_code_t osal_task_destroy(chcnav_task_handle_t task) {
  if (task != nullptr) {
    pthread_join(*(reinterpret_cast<pthread_t *>(task)), nullptr);
    free(task);
  }
