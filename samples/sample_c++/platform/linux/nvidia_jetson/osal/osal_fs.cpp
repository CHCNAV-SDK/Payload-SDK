...
chcnav_return_code_t osal_file_open(const char *file_name, const char *file_mode, chcnav_file_handle_t *file_obj)
{
...
chcnav_return_code_t osal_file_write(chcnav_file_handle_t file_obj, const uint8_t *buffer, uint32_t len, uint32_t *real_len)
{
    int32_t ret;

    if (file_obj == nullptr || buffer == nullptr || len == 0 || real_len == nullptr) {
        return CHCNAV_RETURN_ERR_PARAM;
    }
...
chcnav_return_code_t osal_file_read(chcnav_file_handle_t file_obj, uint8_t *buffer, uint32_t len, uint32_t *real_len)
{
    int32_t ret;
...
chcnav_return_code_t osal_file_seek(chcnav_file_handle_t file_obj, uint32_t offset)
{
    int32_t ret;
...
chcnav_return_code_t osal_dir_open(const char *file_path, chcnav_dir_handle_t *dir_obj)
{
...
chcnav_return_code_t osal_dir_read(chcnav_dir_handle_t dir_obj, CHCNAV_FILE_INFO_STRUCT *file_info)
{
...
chcnav_return_code_t osal_file_rename(const char *oldfile_path, const char *newfile_path)
{
    int32_t ret;
...
chcnav_return_code_t osal_file_stat(const char *file_path, CHCNAV_FILE_INFO_STRUCT *file_info)
{
    struct stat st;
    int32_t ret;
    struct tm *fileTm;
    ...
    fileTm = localtime((const time_t *)&(st.st_mtime));
    if (fileTm == nullptr) {
        return CHCNAV_RETURN_ERR_SYSTEM;
    }
